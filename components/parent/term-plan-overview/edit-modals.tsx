...
const COMMON_SUBJECTS = {
  core: ["Math", "Science", "English", "History", "Foreign Language"],
  extended: ["Art", "Music", "Physical Education", "Computer Science", "Economics"],
}
...
const COMMON_COURSES = {
  ...
  English: ["Literature", "Composition", "Creative Writing", "Speech", "Debate"],
  History: ["World History", "U.S. History", "Government", "Economics", "Geography"],
  "Foreign Language": ["Spanish", "French", "German", "Latin", "Chinese", "Japanese"],
  Art: ["Drawing", "Painting", "Sculpture", "Photography", "Art History"],
  Music: ["Instrumental", "Vocal", "Music Theory", "Music History", "Composition"],
  "Physical Education": ["Team Sports", "Individual Sports", "Fitness", "Health", "Nutrition"],
  "Computer Science": ["Programming", "Web Development", "Game Design", "Robotics", "AI"],
  Economics: ["Microeconomics", "Macroeconomics", "Personal Finance", "Business", "Entrepreneurship"],
}
...
export function SubjectsModal({ open, onOpenChange, studentId, studentName, subjects, onSave }: SubjectsModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-gray-800 text-white border-red-700 max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold flex items-center gap-2 text-white">
            <BookOpen className="h-5 w-5 text-red-400" />
            Edit {studentName}'s Subjects & Courses
          </DialogTitle>
          <DialogDescription className="text-gray-300">Select subjects and courses for this student.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="core" className="space-y-4 mt-4">
              <div className="space-y-2">
                <Label className="text-white">Select Core Subjects</Label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {COMMON_SUBJECTS.core.map((subject) => (
            ...
            <TabsContent value="extended" className="space-y-4 mt-4">
              <div className="space-y-2">
                <Label className="text-white">Select Extended Subjects</Label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {COMMON_SUBJECTS.extended.map((subject) => (
                    <div key={subject} className="flex items-center space-x-2">
                      <Checkbox
                        id={`extended-${subject}`}
                ...
                {[...editSubjects.core, ...editSubjects.extended].length > 0 ? (
                  [...editSubjects.core, ...editSubjects.extended].map((subject) => (
                    <div key={subject} className="space-y-2 p-4 border border-red-700/50 rounded-lg">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {COMMON_COURSES[subject as keyof typeof COMMON_COURSES]?.map((course) => (
                          <Badge
                            key={course}
                            className={`
                              cursor-pointer
                              ${
                                editSubjects.courses[subject]?.includes(course)
                                  ? "bg-red-600 text-white"
                                  : "bg-gray-700 text-gray-300 hover:bg-gray-600"
                              }
                            `}
                            onClick={() => toggleCourse(subject, course)}
                          >
                            {course}
                          </Badge>
                        ))}
                  ...
                  <div className="text-center text-gray-400 py-8">
                    Please select subjects in the Core or Extended tabs first.
                  </div>
                )}
              </div>
            </TabsContent>
